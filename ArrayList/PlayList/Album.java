package org.example.ArrayList.PlayList;
...
public class Album {
    ...
    public Album(String name, String artist) // Add a constructor
    {
        this.name = name;
        this.artist = artist;
    ...
    public boolean addSong(String title, double duration){
        if(findSong(title) != null)//checks if song has already bee added
        {
            return false; // song already added
        }else{
            Song newSong = new Song(title, duration);//checks passed, add new song to album
            songs.add(newSong);
    ...
    public boolean addToPlayList(int trackNumber, LinkedList<Song> playList){
        if(trackNumber <= 0 || trackNumber > songs.size())// TrackNumber starts from 1
        {
            return false; // trackNumber was out of range
        }else{
            Song songToAdd = songs.get(trackNumber - 1);//get the song from album
            String songToAddTitle = songToAdd.getTitle();

            ListIterator<Song> playListIterator = playList.listIterator();
            while(playListIterator.hasNext()){
                if(playListIterator.next().getTitle().compareTo(songToAddTitle) == 0){
                    return false; // Song already exit in playlist
                }
            }
            playList.add(songToAdd);// Add song to the end of playList
            return true;
        }
    }
    public boolean addToPlayList(String title, LinkedList<Song> playList){
        Song songToAdd = findSong(title);// Verify the song exist
        if (songToAdd == null){
